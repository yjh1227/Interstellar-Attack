...
void CPlaneWarView::OnTimer(UINT_PTR nIDEvent)
{
	...
	*/
	...
	if (isStarted == FALSE) {
		...
		cdc.TextOutW(rect.right / 2 - 120, 420, _T("作者：软件工程四班-余俊晖-1652200204（转专业）"));
		cdc.TextOutW(rect.right / 2 - 120, 435, _T("鸣谢：指导老师：黄海霞"));
		cdc.TextOutW(rect.right / 2 - 120, 450, _T("鸣谢：左霞同学p图"));
		HFONT tipFont;
		tipFont = CreateFont(24, 12, 0, 0, 0, 0, 0, 0, 0, 0, 0, 100, 10, 0);
		cdc.SelectObject(tipFont);
		cdc.SetTextColor(RGB(255, 0, 0));
		cdc.TextOutW(rect.right / 2 - 120, 465, _T("点击鼠标左键或空格键开始游戏"));
